...
function createLabyrinth() {
  ...
  for (let r = 0; r < ROWS; r++) {
      for (let c = 0; c < COLS; c++) {
          ...
          // Afegir classes per a caselles inicial/final
          if (r === correctPath[0][0] && c === correctPath[0][1]) {
            cell.classList.add("start-cell");
        }
        if (r === correctPath[correctPath.length - 1][0] && c === correctPath[correctPath.length - 1][1]) {
...
function onCellClick(e) {
  ...
  // Verificar si és adjacent
  const currentPos = nextStepIndex > 0 
    ? correctPath[nextStepIndex - 1] 
    : [-1, -1]; // Cas inicial
    
  const adjacentCells = [
    [currentPos[0] - 1, currentPos[1]], // Amunt
    [currentPos[0] + 1, currentPos[1]], // Avall
    [currentPos[0], currentPos[1] - 1], // Esquerra
    [currentPos[0], currentPos[1] + 1]  // Dreta
  ];

  // Aturar pampallugues quan es clica la casella inicial
  if (cell.classList.contains('start-cell') && nextStepIndex === 0) {
    cell.style.animation = 'none'; // Atura l'animació
  }

  // Permetre clic a la primera casella des de qualsevol posició
  const isFirstStep = nextStepIndex === 0;
  const isAdjacent = adjacentCells.some(([r, c]) => r === row && c === col);
  
  if (!isFirstStep && !isAdjacent) {
    // Efecte de feedback per a moviment no permès
  ...
  // Si la casella ja està marcada com a error, ignorem el click
  ...
  if (row === corrR && col === corrC) {
      ...
      // Marquem la casella com a errònia
...
// Afegim l'animació de vibració al CSS
...
window.addEventListener("resize", checkOrientation); // Per a alguns navegadors
checkOrientation(); // Executar immediatament
